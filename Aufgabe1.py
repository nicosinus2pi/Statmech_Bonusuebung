...
def part1c():
    ...
    for idx, (T, beta) in enumerate(zip(T_values, beta_values)):
        ...
        M_std_err = np.std(measurements, ddof=1) / np.sqrt(N_MC)
        
        # Suszeptibilität: χ = β(⟨M²⟩ - ⟨M⟩²)
        M_var = np.var(measurements, ddof=0)
        chi = beta * M_var
        
        # Fehlerabschätzung
        if N_MC > 1 and M_var > 0:
            chi_error = beta * np.sqrt(2.0 * M_var**2 / (N_MC - 1))
        else:
            chi_error = 0.0
...
def part2c():
    ...
    for idx, T in enumerate(T_values):
        try:
            ...
            M_measurements, M2_measurements = ising.thermalize_and_measure(N_therm, N_MC, k, beta, measure_abs_M=False)
            
            M_mean = np.mean(M_measurements)
            M2_mean = np.mean(M2_measurements)
            
            # Suszeptibilität: χ = β(⟨M²⟩ - ⟨M⟩²)
            chi = beta * (M2_mean - M_mean**2)
            
            # Fehlerabschätzung: Standardfehler der Suszeptibilität
            chi_samples = beta * (M2_measurements - M_mean**2)
            chi_error = np.std(chi_samples, ddof=1) / np.sqrt(N_MC)
            
            # Sicherstellen, dass Fehler nicht zu klein ist für Plot
            chi_error = max(chi_error, np.abs(chi) * 1e-6)
